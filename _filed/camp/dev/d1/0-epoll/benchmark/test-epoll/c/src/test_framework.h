 ...
 * 目标：出错时精确定位到 源文件:行号- 在 printf 语句中用 ANSI 颜色码包裹 "PASS" 和 "FAIL"，如 \033[32mPASS\033[0m 和 \033[31mFAIL\033[0m。
...
#define CHECK(cond, msg) do {                                           \
    if (cond) {                                                         \
        printf("  \033[32mPASS\033[0m | %s:%d | %s\n", __FILE__, __LINE__, msg); \
        __pass++;                                                       \
    } else {                                                            \
        printf("  \033[31mFAIL\033[0m | %s:%d | %s | errno=%d (%s)\n", \
...
#define CHECK_RET(call, expected, msg) do {                             \
    ...
    if (_r == _e) {                                                     \
        printf("  \033[32mPASS\033[0m | %s:%d | %s (ret=%ld)\n",       \
               __FILE__, __LINE__, msg, _r);                            \
        __pass++;                                                       \
    } else {                                                            \
        printf("  \033[31mFAIL\033[0m | %s:%d | %s | expected=%ld got=%ld | errno=%d (%s)\n", \
...
#define CHECK_ERR(call, exp_errno, msg) do {                            \
    ...
    if (_r == -1 && errno == (exp_errno)) {                             \
        printf("  \033[32mPASS\033[0m | %s:%d | %s (errno=%d as expected)\n", \
               __FILE__, __LINE__, msg, errno);                         \
        __pass++;                                                       \
    } else {                                                            \
        printf("  \033[31mFAIL\033[0m | %s:%d | %s | expected errno=%d got ret=%ld errno=%d (%s)\n", \
