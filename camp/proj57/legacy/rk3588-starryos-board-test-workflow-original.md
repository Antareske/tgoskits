...
```text
scripts/rk3588-prepare-starry-rootfs.sh
```

输入参数建议：

- `--device /dev/sdX`：TF 卡块设备。
- `--rootfs tmp/axbuild/rootfs/rootfs-aarch64-alpine.img`：Starry rootfs 镜像。
...
5. 如果没有未分配空间，提示需要缩小 Linux rootfs。
6. 缩小 Linux rootfs 时，先 `e2fsck -f`，再 resize 文件系统，再调整分区尾部。
...
- 镜像直写：`dd if=rootfs-aarch64-alpine.img of=/dev/sdXN conv=fsync`。要求分区大小不小于镜像大小，写完后可按需扩展 ext4。
- 内容复制：挂载 rootfs 镜像和目标分区，把文件复制过去。需要 loop/mount 权限，适合需要扩容或保留目标分区 UUID 的场景。

在 Windows PC + 开发容器环境下，块设备透传可能不稳定。脚本应允许在 WSL 或宿主 Linux 上运行，不强依赖 dev container 内能直接看到 TF 卡。
...
缩小已有 Linux rootfs 是最高风险步骤。脚本必须默认 `--dry-run`，执行前展示：
...
如果 TF 卡尾部已有未分配空间，应优先直接使用，不做缩小。

### Windows + 开发容器设备映射

Windows Docker/WSL 环境下，串口和 TF 卡读卡器映射都可能不稳定。方案应允许：

- 构建和 Starry quick-start 在 dev container 内执行。
- TF 卡分区准备在 WSL 或宿主 Linux 环境执行。
...
2. 手工或半自动在 TF 卡尾部创建 `starry-rootfs` ext4 分区。
3. 写入 rootfs。
4. quick-start `loady` 启动 StarryOS，并显式传 `root=PARTLABEL=starry-rootfs`。
5. 通过串口确认 StarryOS 挂载的是 `starry-rootfs`。
6. 手工通过串口上传一个小脚本并执行。
