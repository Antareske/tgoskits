#!/usr/bin/env python3
"""Export the proj57 ACT checkpoint to a deterministic 2-input ONNX graph.

The exported graph takes `image` ([1,3,224,224], NCHW, ImageNet-normalized)
and `state` ([1,2], QUANTILE-normalized) and returns `action`
([1,chunk,action_dim]) normalized to [-1,1]. The CVAE latent is folded into a
constant buffer from the checkpoint's `inference_latent_mu`, so the graph is
deterministic and has no latent input. This mirrors the QEMU-task export so
RKNN output stays comparable to the existing golden pipeline.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--proj57-root",
        required=True,
        help="Absolute path to proj57 source root containing the act package",
    )
...
def main() -> None:
    ...
    for p in [proj57_root, model_pt, output_onnx, sample_image]:
        if not p.is_absolute():
            raise SystemExit(f"path must be absolute: {p}")
    if not (proj57_root / "act").is_dir():
        raise SystemExit(f"proj57 root missing act package: {proj57_root}")
