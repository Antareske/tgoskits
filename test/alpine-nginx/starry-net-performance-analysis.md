...
>
> 分析基于当前仓库代码（`ax-net 0.7.3`，smoltcp 0.13.1）。所有结论均标注了对应
> 源码位置，便于复核。
...
ax-net socket 层   tcp.rs / udp.rs / raw.rs / unix / vsock
   │  每个 socket 操作内联调用 poll_interfaces()
   ▼
全局服务 SERVICE: Once<Mutex<Service>>     ← 单把大锁
全局套接字 SOCKET_SET: Mutex<SocketSet>     ← 单把大锁
   │
   ▼
...
关键全局对象（`net/ax-net/src/lib.rs:91-96`）：

- `SERVICE: Once<Mutex<Service>>`：整个协议栈一把锁。
- `SOCKET_SET: Mutex<SocketSet>`：所有 socket 一把锁。
- `POLLING_INTERFACES: AtomicBool` + `POLL_AGAIN: AtomicBool`：保证同一时刻只有一个核在跑栈。

**核心判断：当前实现是“单线程协议栈 + 全局大锁 + 同步内联 poll”的模型。** 它在功能上可用、
逻辑清晰，但在性能维度（尤其多核）和稳定性维度（panic 路径、丢包路径）距离 Linux 有系统性差距。
...
### 2.3 同步内联 poll 模型 + 单核串行化

每个 socket 操作（`send`/`recv`/`accept`/`connect`/`poll`）都会内联调用 `poll_interfaces()`
驱动整栈（如 `tcp.rs:483,506,519,567,645`）。`poll_interfaces()` 通过 `POLLING_INTERFACES`
CAS 保证**全局只有一个核真正在 poll，其余直接 return**（`lib.rs:250-268`）。

后果：
- TCP `send` 成功后还会**再 poll 一次**（`tcp.rs:505-507`），加倍了大锁持有。
- 在多任务/多核下，所有网络处理被压成单一临界区串行执行。
- 缺少 NAPI/softirq 那种“中断只标记、批量后处理”的解耦；中断 handler 仅 `wake()`
  PollSet（`device/ethernet.rs:121-129`），唤醒的任务回到同一条 `poll_interfaces` 路径。

**可做的工作：**
- 引入专门的网络 poll 线程 / softirq 任务（现在仅 OOB/SDIO 设备有专属 poll 任务，见
  `lib.rs:437-453`），把数据面与 syscall 上下文解耦，syscall 只读 socket 状态、不驱动整栈。
- 中断合并（interrupt coalescing）+ NAPI 风格 budget 轮询，降低中断频率与 poll 次数。
...
- `SOCKET_SET: Mutex<SocketSet>`：任何 socket 的读写状态都要拿这把锁。
- 锁序固定为 `SERVICE -> SOCKET_SET -> listen entry`（`listen_table.rs:119-120`）。

在 N 核同时收发时，协议栈完全串行，**多核几乎不带来吞吐提升**，反而因锁竞争 + cache
line 争用而下降。`POLLING_INTERFACES` 的单飞设计进一步坐实了“同时只有一个核处理网络”。
...
2. **多队列 + RSS**：驱动开多 RX/TX 队列（现在 `QUEUE_ID0` 单队列，
   `e1000/mod.rs:19`），按流哈希分配到不同核，每核独立 poll 自己的队列。
...
- `LISTEN_TABLE` 是 65536 个 `Arc<Mutex<...>>` 数组（`listen_table.rs:71-81`），端口级锁
  尚可，但 `incoming_tcp_packet` 在 poll 路径里建 socket（`listen_table.rs:165-205`）会在
  持有 SOCKET_SET 锁时分配 64KB×2 缓冲。
- `get_ephemeral_port` 单把 `Mutex<u16>` + 线性扫描（`tcp.rs:844-865`）。
- `udp_bind_check` 遍历全部 socket（`wrapper.rs:41-58`，已标 `TODO: optimize`）。
- `TCP_BOUND_PORTS` 全局 `Mutex<HashMap>` 线性查重（`tcp.rs:799-835`）。
...
（`service.rs:475-500`），高频 poll 下产生分配与 drop。多核下叠加锁竞争更明显。

**可做的工作：** 缓存 / 复用定时器；仅在到期时间变化时才重建。
